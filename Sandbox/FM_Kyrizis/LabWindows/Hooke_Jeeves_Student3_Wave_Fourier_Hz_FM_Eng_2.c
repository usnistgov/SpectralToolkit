...
/* 	waveform modulation */
/*	
/*	13 January 2011	author: Gregory A. Kyriazis (INMETRO - Brazil)		 	   */
...
/*  The author of this software is G.A. Kyriazis (Inmetro - Brazil). */
...
#include <ansi_c.h>
#include <formatio.h>
#include <Hooke_Jeeves_Student3_Wave_Fourier_Hz_FM_Eng_2.h>
...

{
	...
	sis_painel = LoadPanel (0, "Hooke_Jeeves_Student3_Wave_Fourier_Hz_FM_Eng_2.uir", SYSPANEL);
	menu_barra = LoadMenuBarEx (sis_painel, "Hooke_Jeeves_Student3_Wave_Fourier_Hz_FM_Eng_2.uir", MENU, __CVIUserHInst);
	configuracao_painel = LoadPanel (sis_painel, "Hooke_Jeeves_Student3_Wave_Fourier_Hz_FM_Eng_2.uir", CONFIG);	
	medicao_painel = LoadPanel (sis_painel, "Hooke_Jeeves_Student3_Wave_Fourier_Hz_FM_Eng_2.uir", MEDICAO);
	incert_painel = LoadPanel (sis_painel, "Hooke_Jeeves_Student3_Wave_Fourier_Hz_FM_Eng_2.uir", INCERT);
	graficos_painel = LoadPanel (sis_painel, "Hooke_Jeeves_Student3_Wave_Fourier_Hz_FM_Eng_2.uir", GRAFICOS); 
	Selecionar_menu ();
...

{
    ...
    int		   id;
    double	   f1low, f2low, df1, df2, hBest, hWorst;
    double	   h[60][60];
		...
		if (objeto == medicao_painel) {
			switch (item) {
				...
				case MEDICAO_MEDIR_FLICKER:
					...
					break;
				case MEDICAO_FREQ:
					SetCtrlVal (medicao_painel, MEDICAO_LED, TRUE);
					...
					// The next 33 lines (suggested in Matlab script by Allen Goldstein (NIST))					
					// refer to a preliminary search on startpt[1] and startpt[2] to improve 
					// the algorithm convergence.
					// We need to find a good startpoint:
					// startpt[0] = best guess of the carrier frequency
					// startpt[1] = best guess of the modulation phase
					// startpt[2] = best guess of the frequency deviation
					// For the modulated signal, we usually have a pretty good idea
					// of the carrier frequency, however, for the modulated signal, the
					// final modulating signal phase and frequency deviation, are not the
					// same values as those used to generate.
					// Observation of the objective function contour shows us that a
					// grid of 12 points across 0 to 2*pi and 10 points from 0 to 25*DeltaF
					// will have at least one or two good initial guesses.  This
					...
					// Look for a difference of 3000 between zBest and zWorst to stop searching.
					
					GetCtrlVal (medicao_painel, MEDICAO_ID, &id); // id = 20, 40, 60, for DeltaF_start up to 35 Hz, 70 Hz, 105 Hz, respectively. 
					df1 = 4*asin(1)/(id-1);							  
					df2 = 4*4*asin(1)*DeltaF_start*Tsamp/(id-1);	  
					f1low = 0;										             
					f2low = 0;
					startpt[1] = f1low;
					startpt[2] = f2low;
					...
					hWorst = f(startpt, nvars);
					hBest = hWorst;
					for (i=0; i<id; i++) {
   						for (j=0; j<id; j++) {
	   						startpt[1] = i*df1+f1low;
   							startpt[2] = j*df2+f2low;
   							h[i][j] = f(startpt, nvars);
   							if (h[i][j] < hBest) {
   								hBest = h[i][j];
   							}
   							if (h[i][j] > hWorst) {
					...
					GetCtrlVal (medicao_painel, MEDICAO_EPSMIN, &epsilon); /* ending value of stepsize  */
	   				SetCtrlVal (medicao_painel, MEDICAO_LED, FALSE);
	   				jj = hooke(nvars, startpt, endpt, rho, epsilon, itermax);	  /* Two-stage Hooke-Jeeves */
	  			    for (k=0; k<nvars; k++) {
	  			    	omega[k] = endpt[k];
	  			    } 	
	  			    result_freq ();
